...
(function () {
    ...
    function injectFullscreenStyle() {
        ...
        style.textContent = `
            ...
            .ig-fs-btn-t1 { background: rgba(43,48,54,0.5); }
    ...
    function attachFSBtnToVideo(v, btnId, muteBtn, onFSClick, extraBtnClass) {
        ...
        if (muteBtn?.parentElement) {
            ...
            if (slider?.parentElement) {
                ...
                // Compute muteBtn's offset relative to p so coords stay in p's coordinate space.
                const mr = muteBtn.getBoundingClientRect();
                const pr = p.getBoundingClientRect();
                const right  = pr.right - mr.left;
                const bottom = pr.bottom - mr.bottom - 3;
